...
const heroEnterButton = document.querySelector(".hero-enter");
let heroIntroCompleted = false;
let heroIntroStarting = false;
let heroEnterTimer = 0;
...
[heroIntroVideo, heroLoopVideo, heroScrollVideo].forEach(prepareInlineVideo);

const showHeroEnter = () => {
  if (!heroIntroCompleted && heroEnterButton) {
    heroEnterButton.classList.add("is-visible");
  }
};

const hideHeroEnter = () => {
  window.clearTimeout(heroEnterTimer);
  heroEnterButton?.classList.remove("is-visible");
};
...
const startHeroIntro = ({ restart = false, revealOnFailure = true } = {}) => {
  if (!heroIntroVideo || heroIntroCompleted || heroIntroStarting) return;

  heroIntroStarting = true;
  if (restart) {
    try {
  ...
  playHeroVideo(heroIntroVideo)
    ...
    .catch(() => {
      if (revealOnFailure) showHeroEnter();
    })
    .finally(() => {
      heroIntroStarting = false;
    });
};

if (heroIntroVideo && heroLoopVideo) {
  heroIntroVideo.addEventListener("playing", hideHeroEnter);

  heroIntroVideo.addEventListener("ended", () => {
    heroIntroCompleted = true;
    hideHeroEnter();
    hero.classList.add("hero-looping");
    playHeroVideo(heroLoopVideo).catch(() => {});
  });
  ...
  heroEnterButton?.addEventListener("click", (event) => {
    event.preventDefault();
    event.stopPropagation();
    startHeroIntro({ restart: true, revealOnFailure: true });
  });

  // Try the seamless experience first. Only reveal the entry control if
  // WeChat or another embedded browser rejects the autoplay request.
  startHeroIntro({ restart: true, revealOnFailure: true });
  heroEnterTimer = window.setTimeout(() => {
    if (heroIntroVideo.paused && !heroIntroCompleted) showHeroEnter();
  }, 1200);

  document.addEventListener("WeixinJSBridgeReady", () => {
    startHeroIntro({ restart: false, revealOnFailure: true });
  }, { once: true });
}
